...
class Fileupload extends Component {
  ...
  handleAgeInput = e => {
    this.setState({
      pastAge: e.target.value
    });
  };

  handleYearInput = e => {
    this.setState({
      pastYear: e.target.value
    });
  };

  handleTargetInput = e => {
    console.log(typeof this.state.pastYear);
    console.log(typeof this.state.pastAge);
    // const age = parseInt(2020 - this.state.pastYear + this.state.pastAge);
    const age = parseInt("2020") - parseInt(this.state.pastYear) + parseInt(this.state.pastAge);
    this.setState({ targetAge: age });
  };
  ...
  handlePost = async () => {
    ...
    if (selectedFile && targetAge) {
      ...
      try {
        const resultData = await axios.post("http://118.36.53.88:8080/api_v1/cgan", formData);
        const changedImageName = resultData.data.result;
        // const getImage = await axios.get(`http://118.36.53.88:8080/img/${changedImagePath}`);
        this.setState({
          getPicture: changedImageName
        });
      } catch (err) {
  ...
  render() {
    ...
    //   </div>
    // ) : null;
    const filePath = `http://118.36.53.88:8080/img/${this.state.getPicture}`;
    const getImage = this.state.getPicture ? (
      <div>
        <img src={filePath} />
      </div>
